...
public class ConfirmEmailChangeUseCase : IConfirmEmailChangeUseCase
{
    ...
    public async Task<bool> Handle(ConfirmEmailChangeRequest message, IOutputPort<UseCaseResponseMessage> outputPort)
    {
        ...
        {
            await outputPort.Handle(new UseCaseResponseMessage(Guid.Empty, false, $"Invalid Email {message.Email} / Code {message.Code}!", new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Invalid Email {message.Email} / Code {message.Code}!") }));
            return false;
        }
    }
}