...
public class UserRepository : EfRepository<User>, IUserRepository
{
    ...
    public async Task<CreateUserResponse> Create(string firstName, string lastName, string email, string userName, string password)
    {
        try
        {
            ...
            if (!identityResult.Succeeded)
                return new CreateUserResponse(Guid.Parse(appUser.Id), false, identityResult.Errors.Select(e => new Error(e.Code, e.Description)).ToList());
            ...
            return new CreateUserResponse(Guid.Parse(appUser.Id), identityResult.Succeeded, identityResult.Succeeded ? null : identityResult.Errors.Select(e => new Error(e.Code, e.Description)).ToList());
        }
        catch (Exception e)
        {
            _logger.LogCritical($"{nameof(UserRepository)}.{nameof(Create)} Exception! {e.Message}");
            return new CreateUserResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.InternalServerError.ToString(), e.Message) });
        }
    }
    public async Task<DeleteUserResponse> Delete(string userName)
    {
        try
        {
            ...
            if (appUser != null)
            {
                ...
                if (!identityResult.Succeeded)
                    return new DeleteUserResponse(Guid.Parse(appUser.Id), false, identityResult.Errors.Select(e => new Error(e.Code, e.Description)).ToList());
                ...
                if (user != null)
                {
                    ...
                    return new DeleteUserResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.InternalServerError.ToString(), "Failed to remove user from app DB! This is result in inconsistency between application Users table and Identity framework!") });
                return new DeleteUserResponse(Guid.Parse(appUser.Id), identityResult.Succeeded, identityResult.Succeeded ? null : identityResult.Errors.Select(e => new Error(e.Code, e.Description)).ToList());
            }
            else
                return new DeleteUserResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), "Invalid user!") });
        }
        catch (Exception e)
        {
            _logger.LogCritical($"{nameof(UserRepository)}.{nameof(Delete)} Exception! {e.Message}");
            return new DeleteUserResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.InternalServerError.ToString(), e.Message) });
        }
    }
    ...
    {
        ExchangeRefreshTokenResponse response = new ExchangeRefreshTokenResponse(null, null, false);
        try
        {
    ...
    public async Task<SignInResponse> TwoFactorRecoveryCodeSignIn(string recoveryCode)
    {
        try
        {
            ...
            if (!response.Success || response.Id == Guid.Empty || response.User == null || response.Errors!.Any())
            {
            ...
            return new SignInResponse(response.Id, response.User.UserName, true);
        }
        catch (Exception e)
        {
    ...
    public async Task<FindUserResponse> RefreshSignIn(string id)
    {
        try
        {
            ...
            if (appUser != null)
            {
                ...
                return new FindUserResponse(Guid.Parse(appUser.Id), null, true);
            }
            else
                return new FindUserResponse(Guid.Empty, null, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), "Invalid Identity! {id}") });
        }
        catch (Exception e)
        {
            _logger.LogCritical($"{nameof(UserRepository)}.{nameof(Update)} Exception! {e}");
            return new FindUserResponse(Guid.Empty, null, false, new List<Error>() { new Error(HttpStatusCode.InternalServerError.ToString(), e.Message) });
        }
    }
    public async Task<GenerateNew2FARecoveryCodesResponse> GenerateNew2FARecoveryCodes(string id, int codes)
    {
        try
        {
            AppUser? appUser = await _userManager.FindByIdAsync(id);
            if (appUser != null)
                return new GenerateNew2FARecoveryCodesResponse(Guid.Parse(id), (await _userManager.GenerateNewTwoFactorRecoveryCodesAsync(appUser, codes)).ToList(), true);
            return new GenerateNew2FARecoveryCodesResponse(Guid.Empty, null, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Invalid user {id}!") });
        }
        catch (Exception e)
        {
            _logger.LogCritical($"{nameof(UserRepository)}.{nameof(GenerateNew2FARecoveryCodes)} Exception! {e}");
            return new GenerateNew2FARecoveryCodesResponse(Guid.Empty, null, false, new List<Error>() { new Error(HttpStatusCode.InternalServerError.ToString(), e.Message) });
        }
    }
    ...
    public async virtual Task<PasswordResponse> ChangePassword(string id, string oldPassword, string newPassword)
    {
        try
        {
            AppUser? appUser = await _userManager.FindByIdAsync(id);
            if (appUser == null)
                return new PasswordResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), "User not found!") });
            IdentityResult identityResult = await _userManager.ChangePasswordAsync(appUser, oldPassword, newPassword);
            if (identityResult.Succeeded)
                return new PasswordResponse(Guid.Parse(appUser.Id), true, null);
            else
            {
                _logger.LogError($"{nameof(ChangePassword)} failed to change password of user {id}!");
                return new PasswordResponse(Guid.Parse(appUser.Id), false, identityResult.Errors.Select(e => new Error(e.Code, e.Description)).ToList());
            }
        }
        catch (Exception e)
        {
            _logger.LogError($"{nameof(ChangePassword)} Exception! {e.Message}");
            return new PasswordResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.InternalServerError.ToString(), $"User change password failed! {e.Message}") });
        }
    }
    public async Task<PasswordResponse> ResetPassword(string id, string password)
    {
        ...
        if (user != null)
        {
            ...
            if (result.Succeeded)
            {
                ...
                if (!result.Succeeded)
                {
                    ...
                    return new PasswordResponse(Guid.Parse(user.Id), result.Succeeded, result.Succeeded ? null : result.Errors.Select(e => new Error(e.Code, e.Description)).ToList());
                }
            }
            else
            {
        ...
        else
        {
            ...
            return new PasswordResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Trying to reset password of invalid user {id}!") });
        }
    }
    public async Task<PasswordResponse> ResetPassword(string email, string password, string code)
    {
        ...
        else
        {
            ...
            return new PasswordResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Trying to reset password of invalid user {email}!") });
        }
    }
    public async Task<CodeResponse> ForgotPassword(string email)
    {
        try
        {
            ...
            if (user == null)
            {
                ...
                return new CodeResponse(Guid.Empty, null, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Cannot reset password of invalid user {email}!") });
            }
            else if (!await _userManager.IsEmailConfirmedAsync(user))
            {
                ...
                return new CodeResponse(Guid.Empty, null, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Cannot reset password of unconfirmed user email {email}!") });
            }
            ...
            return new CodeResponse(Guid.Parse(user.Id), code, true);
        }
        catch (Exception e)
        {
            _logger.LogCritical($"{nameof(UserRepository)}.{nameof(ForgotPassword)} Exception! {e}");
            return new CodeResponse(Guid.Empty, null, false, new List<Error>() { new Error(HttpStatusCode.InternalServerError.ToString(), e.Message) });
        }
    }
    public async Task<LockUserResponse> LockUser(Guid id) => await LockUser(await _userManager.FindByIdAsync(id.ToString()));
    public async Task<LockUserResponse> LockUser(int id)
    {
        User? user = await _appDbContext.Users.FindAsync(id);
        return user == null ?
                new LockUserResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Invalid user {id}!") })
    ...
    private async Task<LockUserResponse> LockUser(AppUser user)
    {
        if (user != null)
        {
            if (user.LockoutEnabled && user.LockoutEnd > DateTimeOffset.UtcNow)
            {
                ...
                return new LockUserResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Trying to lock an already locked user {user.Id}!") });
            }
            user.LockoutEnabled = true;
            user.LockoutEnd = DateTimeOffset.MaxValue;
            IdentityResult result = await _userManager.UpdateAsync(user);
            return new LockUserResponse(Guid.Parse(user.Id), result.Succeeded, result.Succeeded ? null : result.Errors.Select(e => new Error(e.Code, e.Description)).ToList());
        }
        else
        {
            _logger.LogCritical($"Trying to lock an invalid user {user.Id}!");
            return new LockUserResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Trying to lock an invalid user {user.Id}!") });
        }
    }
    public async Task<LockUserResponse> UnLockUser(Guid id) => await UnLockUser(await _userManager.FindByIdAsync(id.ToString()));
    ...
    public async Task<CodeResponse> GenerateChangeEmailToken(string identityId, string email)
    {
        try
        {
            ...
            if (user == null || !(await _userManager.IsEmailConfirmedAsync(user)))
                return new CodeResponse(Guid.Empty, null, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Cannot change email of invalid user {email}!") });
            // For more information on how to enable account confirmation and password reset please
            // visit https://go.microsoft.com/fwlink/?LinkID=532713
        ...
        try
        {
            ...
            if (appUser != null)
            {
                ...
                return !identityResult.Succeeded ?
                    new FindUserResponse(Guid.Empty, null, false, identityResult.Errors.Select(e => new Error(e.Code, e.Description)).ToList())
                    : new FindUserResponse(Guid.Parse(appUser.Id), await getUser(appUser), identityResult.Succeeded, identityResult.Errors.Select(e => new Error(e.Code, e.Description)).ToList());
            }
            else
                _logger.LogError($"{nameof(UserRepository)}.{nameof(ConfirmEmail)} Invalid user email confirmation! Id: {identityId}, code: {code}");
            return new FindUserResponse(Guid.Empty, null, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Cannot add confirm email of invalid user {identityId}!") });
        }
        catch (Exception e)
        {
    ...
    public async Task<FindUserResponse> ConfirmEmailChange(string identityId, string email, string code)
    {
        try
        {
            ...
            if (appUser != null)
            {
                ...
                if (!identityResult.Succeeded)
                    return new FindUserResponse(Guid.Parse(appUser.Id), null, identityResult.Succeeded, identityResult.Errors.Select(e => new Error(e.Code, e.Description)).ToList());
                ...
                if (!identityResult.Succeeded)
                    return new FindUserResponse(Guid.Parse(appUser.Id), null, identityResult.Succeeded, identityResult.Errors.Select(e => new Error(e.Code, e.Description)).ToList());
                await _signInManager.RefreshSignInAsync(appUser);
                return new FindUserResponse(Guid.Parse(appUser.Id), await getUser(appUser), true, null);
            }
            return new FindUserResponse(Guid.Empty, null, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Cannot confirm email change of invalid user {identityId}!") });
        }
        catch (Exception e)
        {
    ...
    private async Task<LockUserResponse> UnLockUser(AppUser user)
    {
        if (user != null)
        {
            if (!user.LockoutEnabled || !user.LockoutEnd.HasValue || user.LockoutEnd < DateTimeOffset.UtcNow)
            {
                ...
                return new LockUserResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Trying to unlock an unlocked user {user.Id}!") });
            }
            user.LockoutEnabled = true;
        ...
        else
        {
            ...
            return new LockUserResponse(Guid.Empty, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), $"Trying to unlock an invalid user {user.Id}!") });
        }
    }
    ...
    {
        User? user = await getUser(appUser);
        return user == null ? new FindUserResponse(Guid.Empty, null, false, new List<Error>() { new Error(HttpStatusCode.BadRequest.ToString(), "User not found!") }) :
                new FindUserResponse(Guid.Parse(appUser.Id), user, true, null);
    }
